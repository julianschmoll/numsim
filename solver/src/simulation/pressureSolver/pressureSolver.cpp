...
void PressureSolver::setBoundaryValues(DataField &p) {
    ...
    if (partitioning_->ownContainsBoundary<Direction::Bottom>()) {
        ...
        case BoundaryType::Coupled:
            break; // TODO: setStructureBoundaries() macht das eigentlich
        }
    }
        ...
        for (int i = p.beginI() + 1; i < p.endI() - 1; ++i) {
            if (grid_->isSolid(i, j)) {
                ...
                if (fluidCells == 0) {
                    ...
                    p(i, j) = int(bottomIsFluid) * p(i, j - 1) + int(topIsFluid) * p(i, j + 1) + int(leftIsFluid) * p(i - 1, j) +
                              int(rightIsFluid) * p(i + 1, j);
                    p(i, j) /= fluidCells;
                }
            }
        }
