...
void OutputWriterTextParallel::writeFile(double currentTime) {
  ...
  // write mesh width
  file << "nCells: " << grid_->nCells()[0] << "x" << grid_->nCells()[1]
    << ", dx: " << grid_->dx() << ", dy: " << grid_->dy() << std::endl << std::endl;
  ...
  // ---------
  // write header lines
  file << "u (" << grid_->u().size()[0] << "x" << grid_->u().size()[1] << "): " << std::endl
  ...
  // ---------
  // write header lines
  file << "v (" << grid_->v().size()[0] << "x" << grid_->v().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  ...
  // ---------
  // write header lines
  file << "p (" << grid_->p().size()[0] << "x" << grid_->p().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  ...
  // ---------
  // write header lines
  file << "F (" << grid_->u().size()[0] << "x" << grid_->u().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  for (int i = grid_->u().beginI(); i < grid_->u().endI(); i++)
  {
    file << std::setw(fieldWidth) << i;
  ...
  // ---------
  // write header lines
  file << "G (" << grid_->v().size()[0] << "x" << grid_->v().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  for (int i = grid_->v().beginI(); i < grid_->v().endI(); i++)
  {
    file << std::setw(fieldWidth) << i;
  ...
  // ---------
  // write header lines
  file << "rhs (" << grid_->p().size()[0] << "x" << grid_->p().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  for (int i = grid_->p().beginI(); i < grid_->p().endI(); i++)
  {
    file << std::setw(fieldWidth) << i;
